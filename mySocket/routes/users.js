...
let filestorage = multer.diskStorage({
  destination: async (req, file, cb) => {
    let day = sd.format(new Date(), 'YYYYMDD');
    let dir = path.join('public/files/', day);
    // 这是一个异步操作
    await mkdirp(dir);
    cb(null, dir)   // 上传前目录必须存在
  },
...
function deleteAllMsgcode(whereStr) {
  return codeModel.deleteMany(whereStr)
  // return codeModel.remove(whereStr)
}

function codeFindOne(whereStr) {
  return codeModel.findOne(whereStr)
}
...
router.post("/login", (req, res) => {
  ...
  findAll(user).then((data) => {
    ...
    if (data.length > 0) {
      ...
      // 判断验证码是否输入正确
      codeFindOne({ email: req.body.email }).then(msg => {
        console.log('验证码',msg.code , req.body.msgcode)
        if (msg.code !== req.body.msgcode) {
          return res.json({ code: 100, msg: '邮箱验证码不正确' })
        } else {
          // 验证码处理，用完就清空
          deleteAllMsgcode({
            email: req.body.email
            ...
            // console.log(code)
            return res.json({ code: 200, user, msg: '登录成功' })
          }).catch(err => {
            console.log(err)
          })
          // return res.json({ code: 200, user, msg: '登录成功' })
      ...
      return res.json({ code: 100, msg: '不存在此用户' })
    }
  }).catch((err) => {
    console.log(err);
  });
